...
export default function Home() {
	...
	const [monologueByStep, setMonologueByStep] = useState<Map<number, string>>(
		new Map(),
	);
	const [emittedThoughts, setEmittedThoughts] = useState<number[]>([]);
	const [collapsedThoughts, setCollapsedThoughts] = useState<Set<number>>(
		new Set(),
	);
	...
	const startGen = async () => {
		...
		const scheduleToken = (tok: Token) => {
			...
			const t2 = setTimeout(() => {
				if (stoppedRef.current) return;
				setEmittedThoughts((prev) =>
					prev.includes(tok.id) ? prev : [...prev, tok.id],
				);
		...
		esRef.current = openStream(sessionId, {
			...
			onSteeringProposed: (e) => {
				setPendingSteer({
					correction_prompt: e.correction_prompt,
					reason: e.reason,
				});
		...
		if (stoppedRef.current) return;
		const drainEnd =
			nextSlotRef.current + FILLER_TRAVEL_MS + REVEAL_DELAY_MS + 200;
	...
	const divergenceMap = useMemo(() => {
		const m = new Map<
			number,
			{ d: Divergence; phase: "original" | "steered" }
		>();
		if (verdicts.original) {
			verdicts.original.divergences.forEach((d) =>
				m.set(d.pos + phaseOffset("original"), {
					d,
					phase: "original",
				}),
			);
	...
	const activeDivergence = activeToken
		? divergenceMap.get(activeToken.id)?.d
		: undefined;
	...
	const canSubmit =
		phase !== "running" &&
		(mode === "scenario" || promptInput.trim().length > 0);
		...
		<main className={`lyt-grid`}>
			<section className="lyt-block lyt-loose lyt-title-huge lyt-align-left">
				<div className={styles.heroKicker}>
					...
					basado en NLA · Anthropic · publicado hace 72 h ·
					Transformer Circuits 2026
				</div>
				<h1 className={styles.headline}>
					Lo que el modelo dice{" "}
					<span className={styles.headlineAccent}>vs.</span> lo que
					está pensando.
				</h1>
				<p className={styles.heroLead}>
					Auditoría de alineamiento para LLMs open-source. Leemos el{" "}
					<em>residual stream</em> del modelo &mdash; no la cadena de
					pensamiento que escribe sabiendo que la van a leer.
				</p>
				...
				<div className={styles.scenarioCurrentLabel}>
					{currentScenario
						? currentScenario.shortLabel
						: "Free input — escribí tu propio prompt"}
				</div>
				...
				<div className={styles.kpiStrip} aria-label="key product stats">
					<div className={styles.kpiTile}>
						...
						<span className={styles.kpiSub}>
							ai_disclosure · no_pii · financial_advice ·
							sycophancy
						</span>
					</div>
					<div className={styles.kpiTile}>
						...
						<span className={styles.kpiSub}>
							Qwen-2.5-7B · NLA L20 · vast.ai A6000
						</span>
					</div>
					<div className={styles.kpiTile}>
						...
						<span className={styles.kpiSub}>
							zero-shot · sin fine-tune · residual stream
						</span>
					</div>
				</div>
			</section>
			...
			<section
				className={`lyt-block lyt-tight lyt-align-left ${styles.differenceTira}`}
			>
				<p className={styles.differenceTiraTitle}>
					Todos los monitores que conocés están <em>afuera</em> del
					modelo.
					<br />
					<span className={styles.differenceTiraTitleAccent}>
						Verbalize está adentro.
					</span>
				</p>
				<p className={styles.differenceTiraSub}>
					firewalls · output filters · CoT monitors · evals → todos
					black-box. Nosotros leemos activaciones del residual stream.
				</p>
			</section>

			<section
				className={`lyt-block lyt-align-fullbleed lyt-dark lyt-tight ${styles.demoSection}`}
			>
				<div className={styles.demoInner}>
					<header className={styles.demoHeader}>
						<div className={styles.demoHeaderLeft}>
							<div className={styles.demoEyebrow}>
								<span
									className={styles.demoEyebrowDot}
									aria-hidden="true"
								/>
								<span>live · transformer audit</span>
							</div>
							<h2 className={styles.demoTitle}>
								Inferencia bajo observación
								<span className={styles.demoTitleAccent}>
									tokens · activaciones · veredicto
								</span>
							</h2>
						</div>
						<div className={styles.demoMeta}>
							<div className={styles.demoMetaItem}>
								<span className={styles.demoMetaLabel}>
									status
								</span>
								<span
									className={styles.demoMetaValueDot}
									data-state={phase}
								>
									{phase === "running"
										? "streaming"
							...
							<div className={styles.demoMetaItem}>
								...
								<span className={styles.demoMetaValue}>
									{String(
										tokens.filter((t) => !t.isSeparator)
											.length,
									).padStart(3, "0")}
								</span>
							</div>
					...
					<div className={styles.consoleFrame}>
						<span
							className={`${styles.consoleCorner} ${styles.consoleCornerTL}`}
							aria-hidden="true"
						/>
						<span
							className={`${styles.consoleCorner} ${styles.consoleCornerTR}`}
							aria-hidden="true"
						/>
						<span
							className={`${styles.consoleCorner} ${styles.consoleCornerBL}`}
							aria-hidden="true"
						/>
						<span
							className={`${styles.consoleCorner} ${styles.consoleCornerBR}`}
							aria-hidden="true"
						/>

						<div className={styles.stageHeaders} aria-hidden="true">
							<div className={styles.stageHeader}>
								<span className={styles.stageHeaderIdx}>
									01
								</span>
								<span className={styles.stageHeaderName}>
									token stream
								</span>
								<span className={styles.stageHeaderHint}>
									· output
								</span>
							</div>
							<div
								className={`${styles.stageHeader} ${styles.stageHeaderCenter}`}
							>
								<span className={styles.stageHeaderIdx}>
									02
								</span>
						...
						<div
							ref={stageRef}
							className={`${styles.stage} ${showStrip ? styles.stageEndState : ""}`}
						>
							<div
								ref={tokensColRef}
								className={styles.tokensCol}
							>
								<div className={styles.tokensList}>
									{tokens.map((tok) => {
										...
										const consumed = consumedIds.has(
											tok.id,
										);
										...
										const cls = [
											...
											isFlight &&
												tier === "decep" &&
												styles.tokenDeceptiveFlight,
											isFlight &&
												tier === "warn" &&
												styles.tokenWarnFlight,
											tok.phaseLabel === "steered" &&
												styles.tokenSteered,
										]
											.filter(Boolean)
											.join(" ");
										return (
											<motion.span
												...
												ref={(el) => {
													if (el)
														tokenRefs.current.set(
															tok.id,
															el,
														);
													else
														tokenRefs.current.delete(
															tok.id,
														);
												}}
												className={cls}
								...
								{phase === "idle" ? (
									...
									<div className={styles.judgeStack}>
										{verdicts.original && (
											<VerdictCard
												phase="original"
												verdict={verdicts.original}
											/>
										)}
										{verdicts.steered && (
											<VerdictCard
												phase="steered"
												verdict={verdicts.steered}
											/>
										)}
										{steerDelta && (
											<div className={styles.deltaBanner}>
												Steering improvement: trust{" "}
												{steerDelta.original} →{" "}
												{steerDelta.steered} (Δ +
												{steerDelta.delta})
											</div>
										)}
									</div>
					...
					<div className={styles.inspectionLeft}>
						<div className={styles.inspectionHeading}>
							<h2 className={styles.detailTitle}>
								Inspección por token
							</h2>
							<p className={styles.inspectionHint}>
								Pasá el cursor sobre una palabra para leer el
								monólogo interno. Click para fijar.
							</p>
						</div>
						<p
					...
					<div className={styles.bottomDetail}>
						{!activeToken ? (
							<div className={styles.detailEmpty}>
								Hover sobre una palabra para ver monologue +
								divergencia.
							</div>
						) : (
							<div className={styles.detailCard}>
								...
								<div className={styles.monologueBlock}>
									<div className={styles.monologueLabel}>
										Internal monologue
									</div>
									<div className={styles.monologueText}>
										&ldquo;
										{monologueByStep.get(activeToken.id) ??
											activeDivergence?.internal_thought ??
											"—"}
										&rdquo;
									</div>
								</div>
								...
								{activeDivergence && (
									<div className={styles.monologueBlock}>
										...
										<div className={styles.monologueText}>
											{activeDivergence.verbal_claim}
										</div>
										<div
											className={styles.monologueLabel}
											style={{ marginTop: 8 }}
										>
											Categoría
										</div>
										<div className={styles.monologueText}>
											{activeDivergence.category.replace(
												/_/g,
												" ",
											)}
										</div>
			...
			<section className="lyt-block lyt-tight lyt-align-left">
				<h2 className={styles.detailTitle}>
					El landscape de AI safety hoy vive afuera del modelo.
					Excepto este.
				</h2>
				<p className={styles.compareLead}>
					Firewalls, filtros, evals y monitoreo de CoT comparten un
					techo: tratan al modelo como caja negra y leen lo que el
					modelo decidió mostrar.{" "}
					<em>
						Esa señal la escribió el modelo sabiendo que iba a ser
						leída.
					</em>{" "}
					Nosotros leemos el residual stream — la única señal sobre la
					que nunca tuvo presión de entrenamiento.
				</p>
					...
					<table className={styles.compareTable}>
						...
						<tbody>
							<tr className={styles.compareGroupRow}>
								<td colSpan={4}>
									<span className={styles.compareGroupLabel}>
										black-box
									</span>
									<span className={styles.compareGroupHint}>
										afuera del modelo · ven lo que el modelo
										decidió mostrar
									</span>
								</td>
							</tr>
							<tr>
								<td>
									...
									<span className={styles.compareNote}>
										{" "}
										· ej Lakera, Pillar, Prompt Armor
									</span>
								</td>
								<td>Strings de input/output</td>
								<td>
									<span className={styles.compareYes}>
										Sí · el modelo encodea alrededor
									</span>
								</td>
								<td>Block / redact</td>
							</tr>
							<tr>
								<td>
									...
									<span className={styles.compareNote}>
										{" "}
										· ej Apollo, Anthropic AI Control
									</span>
								</td>
							...
							<tr>
								<td>
									...
									<span className={styles.compareNote}>
										{" "}
										· lo que la mayoría va a construir
									</span>
								</td>
							...
							<tr
								className={`${styles.compareGroupRow} ${styles.compareGroupRowOurs}`}
							>
								<td colSpan={4}>
									<span
										className={`${styles.compareGroupLabel} ${styles.compareGroupLabelOurs}`}
									>
										white-box
									</span>
									<span className={styles.compareGroupHint}>
										adentro del modelo · ven lo que el
										modelo está computando
									</span>
								</td>
							</tr>
							<tr>
								...
								<td>Estado oculto</td>
								<td>
									<span className={styles.compareNo}>No</span>
								</td>
								<td>Un escalar por concepto fijo</td>
							</tr>
							<tr className={styles.compareRowOurs}>
								<td>
									<span className={styles.compareUs}>
										verbalize
									</span>
								</td>
								<td>
									<strong>
										Residual stream → texto libre
									</strong>
								</td>
								<td>
									<span className={styles.compareNo}>
										No · señal no-performada
									</span>
								</td>
								<td>
									<strong>
										Auditoría contra cualquier rúbrica
									</strong>
								</td>
							</tr>
						</tbody>
			...
			{pendingSteer && (
				<div className={styles.modalOverlay}>
					<div className={styles.modalCard}>
						...
						<pre className={styles.modalCorrection}>
							{pendingSteer.correction_prompt}
						</pre>
						<div className={styles.modalCountdown}>
							Timeout: {steerCountdown}s
						</div>
				...
				<div>
					...
					<ul className={styles.verdictDivList}>
						{verdict.divergences.map((d, i) => (
							<li
								key={i}
								className={styles.verdictDivItem}
