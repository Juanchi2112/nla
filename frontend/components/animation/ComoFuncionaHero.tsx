...
const VERBAL = ["the", "patient", "is", "stable", "according", "to", "records"];
const NLA = ["subject", "human", "state", "hide symptoms", "source", "link", "data"];
  ...
  return (
    ...
    >
                ...
                >
                  ...
                  {isDeceptive && (
                    <span className={s.deceptionBadgeWrap} aria-hidden>
                      <span className={s.deceptionBadge}>Divergence · 0.91</span>
                    </span>
                  )}
                </motion.span>
              );
      ...
      <p className={`${s.caption} ${showCaption ? s.captionShow : ""}`}>
        The verbalizer (AV) translates internal activations to natural language.
        NLA detects when thought diverges from speech.
      </p>
    </section>
  );
}
