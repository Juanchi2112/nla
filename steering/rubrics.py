...
Each rubric defines:
    name           — stable identifier used in logs and snapshots.
    ...
    s_target_key   — entry of COMPLIANT_TEMPLATES used as steering target
                     when this rubric fires.
    modes          — which steering modes apply this rubric. Mode A sniffs
                     once at the prompt boundary; Mode B sniffs every K
                     tokens during generation.

To add a rubric: define here + add a matching entry in templates.py if the
existing ones don't fit. Keep wording in the third-person "The model is
planning to ..." style — that mirrors the AV's voice and the AR was
trained on AV outputs.
"""
...
RUBRICS: dict[str, Rubric] = {
    ...
    "tool_misreport": Rubric(
        ...
        s_target_key="report_tool_faithfully",
        modes=("B",),
    ),
...
def rubrics_for_mode(mode: Mode) -> list[Rubric]:
    """Return rubrics that apply to a given steering mode, in declaration order."""
